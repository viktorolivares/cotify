<template>
  ...
  <form @submit.prevent="store" class="grid grid-cols-1 md:grid-cols-3 gap-10">
    ...
    <div class="md:col-span-1 bg-white dark:bg-indigo-900 rounded-md shadow">
      <div class="flex flex-wrap -mb-8 -mr-6 p-8">
        ...
        <div class="my-3 w-full">
          ...
          <div class="relative">
            <select class="form-select dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
              v-model="form.account_id" disabled>
        ...
        <div class="my-3 w-full">
          ...
          <div class="relative">
            <select class="form-select dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
              v-model="form.template_id" :error="form.errors.template_id">
              <option value="" disabled>Select a template</option>
              <option v-for="template in templates" :value="template.id" :key="template.id">
                {{ template.description }}
              </option>
            </select>
            <span class="form-error" v-if="form.errors.template_id">
        ...
        <!-- Logo -->
        <file-input class="my-3 w-full" v-model="form.logo" :error="form.errors.logo"
          accept="image/jpeg,image/png,image/jpg" label="Logo" />
      </div>
    </div>
  </form>
</template>
